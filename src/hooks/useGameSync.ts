...
import { subscribeToGameUpdates, notifyGameUpdate, GameSyncMessage } from '@/lib/supabase';
...
export const useGameSync = ({ gameId, onGameUpdate, onGameEnd, enabled = true }: UseGameSyncOptions) => {
  ...
  useEffect(() => {
    ...
    const channel = subscribeToGameUpdates(gameId, (message: GameSyncMessage) => {
      // Only react to other players' actions (not our own)
      if (message.playerId !== address) {
        console.log(`🔄 Other player action detected: ${message.action}`);
        onGameUpdate(); // Trigger immediate game state refresh
        
        // If this is a move reveal, the game has ended - notify callback
        if (message.action === 'move_revealed' && onGameEnd) {
          console.log(`🎯 Game ended via real-time: ${message.action}`);
  ...
  // Function to notify other players when we make a move
    ...
    const notifyMove = async (action: GameSyncMessage['action']) => {
      ...
      console.log(`📢 Notifying other players: ${action} at ${new Date().toISOString()}`);
