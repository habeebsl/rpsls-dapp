...
export function useRevealMove({
  ...
  const handleRevealMove = async () => {
    ...
    try {
      ...
      const moveNumber = MOVE_TO_NUMBER[userMove as keyof typeof MOVE_TO_NUMBER];
      
      if (moveNumber === undefined) {
        throw new Error(`Invalid move: ${userMove}`);
      }

      console.log('Revealing move:', userMove, 'with move number:', moveNumber);
      
      // Call the solve function with the original move and salt
      const transaction = await solve(contractAddress, moveNumber, gameResultData.salt, signer);
      ...
      const outcome = await determineGameOutcome(contractAddress, moveNumber, gameState.c2, signer);
      ...
      if (outcome !== 'tie') {
        const winner = outcome === 'j1-wins' ? currentUserAddress : opponentAddress;
        const loser = outcome === 'j1-wins' ? opponentAddress : currentUserAddress;
        
        await notificationHelpers.gameCompleted(loser, winner, contractAddress, 'won');
        await notificationHelpers.gameCompleted(winner, loser, contractAddress, 'lost');
      } else {
        // Both players tied
        await notificationHelpers.gameCompleted(opponentAddress, currentUserAddress, contractAddress, 'tied');
        await notificationHelpers.gameCompleted(currentUserAddress, opponentAddress, contractAddress, 'tied');
      }

      // Return result data for parent to handle modal
      return {
        result: j1Result,
        playerMove: userMove,
        opponentMove: NUMBER_TO_MOVE[gameState.c2],
      ...
      if (error instanceof Error) {
        if (error.message.includes('insufficient funds')) {
          ...
          errorMessage = 'Game data not found. Please try refreshing the page.';
        } else {
