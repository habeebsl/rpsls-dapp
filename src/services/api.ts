...
export const gameApi = {
     ...
     */
    storeGameResult: async (
        data: StoreGameResultRequest
    ): Promise<StoreGameResultResponse> => {
        const response = await api.post<StoreGameResultResponse>(
            '/games/store-result',
            data
        );
    ...
    ): Promise<UserMoveResponse> => {
        const response = await api.get<UserMoveResponse>(
            `/games/${gameId}/user-move/${playerAddress}`
        );
        return response.data;
    },
    ...
    getGameResult: async (gameId: string, playerAddress: string) => {
        const response = await api.get(
            `/games/${gameId}?playerAddress=${playerAddress}`
        );
        return response.data;
    },
...
export const notificationApi = {
    ...
    loadNotifications: async (
        ...
        const response = await api.get<GetNotificationsResponse>(
            `/notifications?address=${userAddress}`
        );
        return response.data;
    },
